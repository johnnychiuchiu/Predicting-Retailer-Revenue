...
fit_multiple <- lm(logtarg~.-id - total_duration+ avg_qty:ordperyr:avg_ord_value + avg_qty:coe_va + total_duration:total_money, data = train)
...
lasso_feature = c('logtarg','days_recent_purchase', 'days_first_purchase', 'order_count', 'avg_qty',
                 'avg_ord_value','slope','coe_va','cat_count','entrophy','total_money','avg_cats',
                 'distinct_orddate','aug_after_orders','ordperyr')
fit_lasso = lm(logtarg~., data = train[lasso_feature])
summary(fit_lasso) #Adjusted R-squared:  0.01626


##### Model selection
# choose the model according to Root Mean Square Error, Adjusted R-squared.
# stepwise backward regression give us the best result with 
# Residual standard error: 0.6112
# Adjusted R-squared:  0.01422 
# Therefore, we choose it as the final model.
